...
const Book = ({setBooks, bookId, bookTitle, bookStart, bookEnd}) => {
    ...
    const onDeleteBook = async (id) => {
        ...
        setBooks((previousBooks) => previousBooks.filer((book) => book.id !== id));
    };
    ...
    return (
        <form onSubmit={(e) => {
            ...
            <Stack key={bookId}>
                <TextField id="standard-basic" label="Title" defaultValue={bookTitle} variant="standard" onChange={e => setNewUpdateBookTitle(e.target.value)} />
                <LocalizationProvider dateAdapter={AdapterDateFns}>
                    <DatePicker label="Start Date" openTo="day" views={['year', 'month','day']} value={new Date(updateNewBookStart)} onChange={(newValue) => {
                        setNewUpdateBookStart(newValue.toISOString())
                    }}
                    slotProps={{ textField: { variant: 'outlined' }}}/>
                    <DatePicker label="End Date" openTo="day" views={['year', 'month', 'day']} value={new Date(updateNewBookEnd)} onChange={(newValue) => {
